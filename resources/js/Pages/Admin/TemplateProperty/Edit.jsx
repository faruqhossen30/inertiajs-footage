...
export default function Edit({ templateProperty, templates, propertyTypes }) {
    const { data, setData, put, processing, errors } = useForm({
        template_id: templateProperty.template_id?.toString() || '',
        key: templateProperty.key || '',
        value: templateProperty.value || '',
        type: templateProperty.type || '',
        sort_order: templateProperty.sort_order || 0,
    });

    const [selectedPropertyType, setSelectedPropertyType] = useState(null);

    useEffect(() => {
        const propertyType = propertyTypes.find(pt => pt.slug === templateProperty.type);
        setSelectedPropertyType(propertyType);
    }, [templateProperty.type, propertyTypes]);
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('admin.template.property.update', templateProperty.id));
    };
    ...
    return (
        <AuthenticatedLayout>
            <Head title={`Edit ${templateProperty.key}`} />
            ...
            <div className="space-y-6">
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight">Edit Template Property</h1>
                        <p className="text-muted-foreground">
                            Update the property: {templateProperty.key}
                        </p>
                    </div>
                </div>
